...
# (Recycapp, Klyde, Cycle en Bray, Bennes Pro, Pointeuse, Feedback, Recyc Caisse,
# Bâtire, Mes Todo).
#
# Pourquoi : les 10 apps partagent UN seul déploiement Convex. Le dossier
# `convex/` de Mes Outils (`~/mesoutils`) est la SEULE source de vérité ; c'est
# un sur-ensemble qui contient les fonctions des 10 apps. Les copies dans les
...
SIBLINGS=( "$HOME/recycapp" "$HOME/klyde" "$HOME/cycleenbray" "$HOME/bennepro" "$HOME/pointeuselsdb" "$HOME/feedback" "$HOME/recyccaisse" "$HOME/batire" "$HOME/mestodo" )
